  ...
  async function handleDelete(id){
    ...
    return(
      ...
      <form className="post-form" onSubmit={handleSubmit}>
          ...
          <button className="post-button" disabled={!props.user}>{postState.editMode ? 'EDIT POST' : 'POST'}</button>
      </form>

      {props.user ? postState.posts.map((p, i) => (
        <article className="main-post-box" key={i}>
          ...
          <form className="add-comment-form"  onSubmit={handleCommentSubmit} >
            ...
            <button className="comment-button" disabled={!props.user}>{'REACT'}</button>
          </form>
          {props.user ? postState.comments.map((c, i) => (
            <p key={i}>{c.comment}</p>
          )):
